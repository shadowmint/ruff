...
class TestRunner(object):
  ...
  def render(self):
    ...
    for r in self.results:
      ...
      if out[0]:
        passed += out[1]
        logging.info(Color.OKGREEN + 'PASSED' + Color.ENDC + ': {0}/{1}: {2}'.format(out[1], out[1], r))
    for r in self.results:
      out = self.results[r]
      if not out[0]:
        logging.info(Color.FAIL + 'FAILED' + Color.ENDC + ': {0}/{1}: {2}'.format(out[2], out[1], r))
        passed += out[1] - out[2]
    if passed == total:
      logging.info(Color.OKBLUE + 'PASSED' + Color.ENDC + ': {0} tests passed'.format(passed, total))
    else:
      logging.info(Color.FAIL + 'FAILED' + Color.ENDC + ': {0}/{1} tests failed'.format(total - passed, total))
    result = passed == total
